...
class RankingService {
     ...
     */
    static async calculateRank(classId, yearId, period, targetStudentId) {
        ...
        const studentsWithAverages = enrollments.map((enrollment) => {
            const totalPoints = enrollment.grades.reduce((acc, g) => acc + (Number(g.value) * g.coeff), 0);
            const totalCoeffs = enrollment.grades.reduce((acc, g) => acc + g.coeff, 0);
        ...
        const rankIndex = studentsWithAverages.findIndex((s) => s.studentId === targetStudentId);
        const rank = rankIndex !== -1 ? rankIndex + 1 : 0;
        // 5. Calculer la moyenne de la classe
        const classAverageTotal = studentsWithAverages.reduce((acc, s) => acc + s.average, 0);
